...
pub async fn serial_handler(State(state): State<WebState>, Path(port_label): Path<String>, ws: WebSocketUpgrade) -> Response {
...
async fn serial_handle_socket(mut socket: WebSocket, state: WebState, port_label: String) {
    ...
    socket.send(Message::Binary(Bytes::from(port.log_buffer))).await.unwrap();

    while let Ok(message) = receiver.recv().await {
        match message {
             MQTTBroadcast::SerialData { label, data } => {
                 if label == port_label {
                     socket.send(Message::Binary(Bytes::from(data))).await.unwrap();
                 }
             }
            _ => {}
        }
